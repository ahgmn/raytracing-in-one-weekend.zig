...
pub fn render(self: *const @This(), world: *const hittable.List, stdout: anytype, stderr: anytype) !void {
    ...
    for (0..self.image_height) |row| {
        ...
        for (0..self.image_width) |col| {
            ...
            const pixel_center = self.pixel00_loc + (self.pixel_delta_u * vec.to(colf)) + (self.pixel_delta_v * vec.to(rowf));
...
pub fn init(image_width: usize, aspect_ratio: f32) @This() {
    ...
    const pixel_delta_u = viewport_u / vec.to(mh.toF32(usize, image_width));
    const pixel_delta_v = viewport_v / vec.to(mh.toF32(usize, image_height));

    const viewport_upper_left = camera_center - Vec3{ 0, 0, focal_length } - (viewport_u / vec.to(2)) - (viewport_v / vec.to(2));

    const pixel00_loc = viewport_upper_left + (pixel_delta_u + pixel_delta_v * vec.to(0.5));
...
fn rayColor(ray: *const Ray, world: *const hittable.List) Color3 {
    ...
    if (hit_record) |rec| {
        return vec.to(0.5) * (rec.normal + Color3{ 1, 1, 1 });
    }
    const unit_direction = vec.unit(ray.dir);
    const a = 0.5 * (unit_direction[1] + 1.0);
    return vec.to(1.0 - a) * Color3{ 1, 1, 1 } + vec.to(a) * Color3{ 0.5, 0.7, 1 };
}
